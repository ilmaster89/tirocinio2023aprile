...
public class MainController {
	...
	public String register(@RequestParam String name, @RequestParam String surname, @RequestParam String email) {
		...
		MailUtil.sendMail(email, htmlString, null);
	...
	public String order(HttpServletResponse response) {
		...
		try {
			...
			MailUtil.sendMail("alberto.ramponi@gmail.com", "<h1>La tua ricevuta</h1>",
					"receipt" + name + surname + ".pdf");
	...
	@GetMapping("/download")
	public String downloadReceipt(HttpServletResponse response) {
		...
		// \"yourReceipt.pdf\"");
		response.addHeader(HttpHeaders.CONTENT_DISPOSITION, "inline; filename = \"yourReceipt.pdf\"");
		try {
